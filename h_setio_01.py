...
Original file is located at
    https://colab.research.google.com/drive/1TgFPdrLIhHDjQNpw-W5YkKyjX2HQTVyI
"""
...
def f_regr(b, formula=None,dependent_dropdown=None, independent_dropdown=None):
    with output2:
        ...
        try:
            ...
            # print(df.head())
            # residual plot
            # plt.scatter(df['predicted'], df['error'])
            ...
            # plt.hist(df['error'], bins=10, edgecolor='k')
            # plt.xlabel('Residu')
            # plt.ylabel('Frequency')
            ...
            axes[0].scatter(df['predicted'], df['error'])
            ...
            axes[1].hist(df['error'], bins=10, edgecolor='k')
            ...
            # Create a new DataFrame of only the independent variables (including constant)
            X = model.model.exog  # matrix of predictors used in the formula
            vif_df = pd.DataFrame()
            vif_df['variable'] = model.model.exog_names  # includes 'Intercept'
